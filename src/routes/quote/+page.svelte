<script lang="ts">
  ...
  const indoorOptions = [
    ...
    {
      value: "smart-home",
      label: "Smart home devices (lights, thermostats, cameras)",
    },
  ];
...
<main class="min-h-screen bg-zinc-950 text-white">
  ...
  <div class="max-w-5xl mx-auto px-6 pt-24 pb-16">
    ...
    <div class="text-center mb-12 py-4">
      <p class="text-3xl text-blue-300 mt-3">
        Quote form coming soon... Please <a
    ...
    <form method="POST" use:enhance class="space-y-10" inert>
      ...
      <div class="bg-zinc-900 rounded-3xl p-8">
        ...
        <div class="grid md:grid-cols-2 gap-5">
          <TextInput {form} field="fullName" label="Full Name" />
          <TextInput {form} field="company" label="Company" />
          <TextInput {form} field="phone" label="Phone" type="tel" />
          <TextInput {form} field="email" label="Email" type="email" />
          <TextInput {form} field="jobTitle" label="Job Title / Role" />
        </div>
      </div>
      ...
      <div class="bg-zinc-900 rounded-3xl p-8">
        ...
        <div class="grid md:grid-cols-2 gap-5">
          ...
          <TextInput
            {form}
            field="numberOfHomes"
            label="Number of Homes in this Project"
            type="number"
          />
          <TextInput
            {form}
          ...
          <TextInput
            {form}
            field="roughInPhase"
            label="Rough-in Phase"
            placeholder="e.g. Electrical complete"
          />
          <TextInput
            {form}
            field="totalHomesPlanned"
            label="Total Homes Planned (Next 12 months)"
            type="number"
          />
        </div>
      </div>
      ...
      <div class="bg-zinc-900 rounded-3xl p-8">
        ...
        <div class="grid md:grid-cols-2 gap-5 mb-6">
          <TextInput
            {form}
            field="squareFootage"
            label="Square Footage (Total conditioned space)"
            type="number"
        ...
        <div class="mb-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <TextInput
              {form}
              field="numberOfUnits.mainHouse"
              label="Main House"
              type="number"
            />
            <TextInput
              {form}
              field="numberOfUnits.guestHouse"
              label="Guest House / ADU"
              type="number"
            />
            <TextInput
              {form}
              field="numberOfUnits.garage"
              label="Garage / Workshop"
              type="number"
            />
            <TextInput
              {form}
              field="numberOfUnits.poolHouse"
              label="Pool House / Outdoor Kitchen"
              type="number"
            />
          </div>
        </div>
        ...
        <Textarea
          {form}
          field="specialNotes"
          label="Special Construction Notes"
          placeholder="E.g. metal framing, thick stone walls, smart home integration, etc."
          rows={3}
        />
      </div>
      ...
      <div class="bg-zinc-900 rounded-3xl p-8">
        ...
        <div class="mb-6">
          <label class="flex items-center gap-3 text-sm cursor-pointer">
            <input
              type="checkbox"
              bind:checked={$formData.needsPoeCabling as any}
              class="accent-blue-600 scale-125"
            />
        ...
        <TextInput
          ...
          label="Any specific brand preferences? (e.g. Ubiquiti, Aruba, TP-Link Omada)"
        />
      </div>
      ...
      <div class="bg-zinc-900 rounded-3xl p-8">
        ...
        <div class="grid md:grid-cols-2 gap-5 mb-6">
          <Select
            {form}
            field="budgetRange"
            label="Budget Range for WiFi System"
    ...
    {#if $formMessage}
      <div class="mt-8 bg-zinc-900 rounded-3xl p-12 text-center">
        <div
          class="mx-auto w-16 h-16 bg-emerald-500/10 rounded-full flex items-center justify-center mb-6"
        >
          <span class="text-3xl">✓</span>
        </div>
        <h2 class="text-2xl font-semibold mb-3">Request Received</h2>
        <p class="text-zinc-400 max-w-md mx-auto">{$formMessage}</p>
        <a href="/" class="inline-block mt-6 text-blue-400 hover:text-blue-300"
          >← Back to Home</a
        >
      </div>
    {/if}
